...
def singleBookCrawl(booktezhengList,searchSiteList):
    workingSite=0
    (zhuyeurl,zhuyehtml_str,zhuyebookname,zhuyebookauther,zhuyebookintroduction)=singleBookzhuyeCrawl(booktezhengList[workingSite],searchSiteList[workingSite])
    startTime=datetime.datetime.now()
    print('Start getting data at '+startTime.strftime( '%H:%M:%S' ))
    errorChapter=[]#用于记录出错章节名
    for i in range(len(zhuyehtml_str)):#遍历章节
        ...
        if success==True:
            ...
            chapterName=re.findall('html">(.*?)</a></dd>',zhuyehtml_str[i],re.S)#正则抓取章节名
            print('Error: '+chapterName[0])
            errorChapter.append(chapterName[0])
    endTime=datetime.datetime.now()
    deltaTime=(endTime-startTime).seconds
